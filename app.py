...
def summarize_text(text):
    sentences = sent_tokenize(text)
    if len(sentences) > 5:  # Simple summarization: First 5 sentences
...
def index():
    if request.method == 'POST':
        ...
        # Handle text input from the textarea if no file was uploaded
        elif request.form.get('text_input'):
            text = request.form.get('text_input').strip()

        else:
            return "Please provide either a file or text input."
